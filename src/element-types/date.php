...
namespace awsmug\TorroFormsDoubleOptIn\Element_Types;
...
class Date extends Element_Type {
	...
	public function filter_json( $data, $element, $submission = null ) {
		...
		$data['date_labels'] = array(
			'year'  => __( 'Year', 'torro-forms-double-opt-in' ),
			'month' => __( 'Month', 'torro-forms-double-opt-in' ),
			'day'   => __( 'Day', 'torro-forms-double-opt-in' ),
		);
	...
	public function validate_field( $value, $element, $submission ) {
		...
		if ( ! empty( $settings['required'] ) && ( empty( $year ) || empty( $month ) || empty( $day ) ) ) {
			return $this->create_error( 'value_required', __( 'You must enter something here.', 'torro-forms-double-opt-in' ), $parsed_value );
		}

		if ( $month < 1 || $month > 12 ) {
			return $this->create_error( 'month_out_of_boundaries', __( 'The value for the month must be between 1 and 12.', 'torro-forms-double-opt-in' ), $parsed_value );
		}

		if ( $day < 1 || $month > 31 ) {
			return $this->create_error( 'day_out_of_boundaries', __( 'The value for the day must be between 1 and 31.', 'torro-forms-double-opt-in' ), $parsed_value );
		}
		...
		if ( $year < $min_year || $year > $max_year ) {
			...
			return $this->create_error( 'year_out_of_boundaries', sprintf( __( 'The value for the year must be between %1$s and %2$s.', 'torro-forms-double-opt-in' ), $min_year, $max_year ), $parsed_value );
		}
	...
	protected function bootstrap() {
		$this->slug        = 'date';
		$this->title       = __( 'Date', 'torro-forms-double-opt-in' );
		$this->description = __( 'A dropdown-based date field element.', 'torro-forms-double-opt-in' );
		...
		$this->settings_fields['min_year'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Minimum year', 'torro-forms-double-opt-in' ),
		...
		$this->settings_fields['max_year'] = array(
			'section'       => 'settings',
			'type'          => 'number',
			'label'         => __( 'Maximum year', 'torro-forms-double-opt-in' ),
			'input_classes' => array( 'small-text' ),
			'default'       => current_time( 'Y' ),
			'min'           => 1,
			'step'          => 1,
		);
		$this->settings_fields['test_setting'] = array(
			'section' => 'settings',
			'type' => 'datetime',
			'store' => 'date',
			'label' => __( 'Date field test.', 'cmwp-torro-elements' ),
			'default' => '',
			);
